...
2. **Does it belong to exactly one product feature?**  
   ...
   → **`src/features/<feature>/`** — typically `ui/`, `queries/`, `mutations/`, `actions/`, `validations.ts` (or `validations/`), optional `domain/`, `server/`, `services/`, `context/`, `hooks/`, `types.ts`, `index.ts`.
...
- **Standard subfolders** (use when you have enough files to justify them):
  ...
  | `queries/`                         | Blitz **read** RPC — each implementation file must `export default` a resolver (see below) |
  | `mutations/`                       | Blitz **write** RPC — same default-export rule                                             |
  | `actions/`                         | Next **`"use server"`** actions / workflows with named async exports                       |
...
- **`queries/`** — Blitz read RPC; **`mutations/`** — Blitz write RPC. These are transport wrappers and each implementation file must have a default resolver export.
- **`actions/`** — Next **`"use server"`** workflows with named async exports. Use these for multi-step server workflows called from client components when Blitz RPC is not the right fit.
- **`server/`** holds **private** helpers used only on the server.

|                 | `queries/` / `mutations`                    | `actions/`                          | `server/`                     |
| --------------- | ------------------------------------------- | ----------------------------------- | ----------------------------- |
| **Role**        | Public Blitz RPC transport                  | Public server-action workflow       | Private implementation        |
| **Called from** | Client via Blitz `useQuery` / `useMutation` | Client components or server modules | **Only** other server modules |

---

## Blitz RPC: scanner rules and gotchas

Blitz scans **`features/*/queries/*`** and **`features/*/mutations/*`** and wraps each file’s **default export** as an RPC handler.
...
2. **Do not put non-resolver modules** (shared Prisma selects, plain constants-only files, RSC-only helpers) **inside** `queries/` or `mutations/`. Place them at the **feature root** (e.g. `inviteSelect.ts`), in **`domain/`**, or in **`server/`**, depending on purity and IO.
