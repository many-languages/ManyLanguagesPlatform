...
function buildExtractionOutputHash(
  bundle: ReturnType<typeof extractVariableBundleFromResults>
): string {
...
export async function approveExtraction(input: {
  studyId: number
}): Promise<{ extractionSnapshotId: number; variableCount: number }> {
  return withStudyWriteAccess(input.studyId, async () => {
    const study = await db.study.findUnique({
      where: { id: input.studyId },
      select: {
        id: true,
    ...
    return db.$transaction(async (tx) => {
      ...
      if (!existingSnapshot && bundle.variables.length > 0) {
        await tx.studyVariable.createMany({
          data: bundle.variables.map((variable) => ({
