...
import { deleteJatosStudy } from "./deleteStudy"

const mockFetch = vi.fn()
global.fetch = mockFetch

describe("deleteJatosStudy", () => {
  ...
  it("uses passed token when provided", async () => {
    ...
    await deleteJatosStudy("123", { token: "custom-token-xyz" })
  ...
  it("throws when auth.token is missing", async () => {
    await expect(deleteJatosStudy("123", { token: "" })).rejects.toThrow(
